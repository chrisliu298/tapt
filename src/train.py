...
if __name__ == "__main__":
    ...
    df["tokens"] = df["text"].progress_apply(lambda x: gpt2_tokenizer.encode(" "+x, return_tensors="pt").to(device)[0, :config["txt_in_len"]])
    ...
    control_tokens = dict((s, gpt2_tokenizer.encode(s, return_tensors="pt").squeeze().to(device)) for s in control_str)
    ...
    for epoch in tqdm(range(int(np.ceil(config["steps"]/config['batch_size'])))):
        ...
        sentiment_inputs, attention_masks = build_bert_batch_from_txt(texts, bert_tokenizer, device)    
        ...
        pos_logits = []
        for i in range(int(config['batch_size']/fbs)):
            res = bert_model.forward(sentiment_inputs[i*fbs:(i+1)*fbs],
        ...
        for control_s in control_str:
            ...
            logs[key] = np.mean([r for r, t in zip(logs['env/reward_dist'], task_list) if t==control_s])
        # wandb.log(logs)
